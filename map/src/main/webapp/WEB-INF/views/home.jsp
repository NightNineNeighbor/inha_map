...
<script>
	window.onload = function() {
		...
		document.getElementById("loadNode").addEventListener(
				"click",
				function() {
					loadNode($("#nodesInfo").val(), $("#graphInfo").val(), $(
							"#selectableInfo").val())
				})
		...
		document.getElementById("findPath").addEventListener(
				...
				function() {
					$.ajax({
						url : "/map/dijkstra",
						type : "post",
						data : "json=" + JSON.stringify(graph) + "&nodeAmount="
								+ String(Object.keys(nodes).length)
		...
		document.getElementById("endOfSetting").addEventListener(
				"click",
				function() {
					showSelectableList();
				});
		function showSelectableList(){
			$("#selectableList").text("");
			$.each(selectableNode, function(key, value) {
				$("#selectableList").append(
						"[ " + key + ", " + value + " ] ");
			});
		}
		...
		document.getElementById("mapToggle").addEventListener("click",
				function() {
					if (markers[0].map === map) {
						toggleGraphNode(null);
					} else {
						toggleGraphNode(map);
					}
				});

		function toggleGraphNode(param) {
			$.each(markers, function(key, value) {
				value.setMap(param);
			})
			for (var i = 0, ii = polylines.length; i < ii; i++) { //polyline 모두 삭제
				polylines[i].setMap(param);
			}
		}
		...
		function inite() {
			loadNode('${nodes}', '${graph}', '${selectable}');
			toggleGraphNode(null);
			showSelectableList();
		}

		function loadNode(nodesInfo, graphInfo, selectableInfo) {
			$.each(markers, function(key, value) {
				value.setMap(null);
			});
			markers = {};
			...
			selectableNode = JSON.parse(selectableInfo) //TODO
			$.each(selectableNode, function(key, values) {
				var icon = {
					url : './resources/j.png',
					size : new naver.maps.Size(24, 37),
					anchor : new naver.maps.Point(12, 37),
		...
		function makeBuilding() {
			...
			var tileSize = new naver.maps.Size(256, 256), proj = {
